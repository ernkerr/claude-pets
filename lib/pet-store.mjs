...
export function reorderPet(fromId, toId) {
  const store = loadStore();
  const fromIdx = store.pets.findIndex((p) => p.id === fromId);
  const toIdx = store.pets.findIndex((p) => p.id === toId);
  if (fromIdx === -1 || toIdx === -1 || fromIdx === toIdx) return null;
  const [pet] = store.pets.splice(fromIdx, 1);
  store.pets.splice(toIdx, 0, pet);
  saveStore(store);
  return listPetsResolved();
}
